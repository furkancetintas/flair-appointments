    ...
    <header className="h-16 border-b bg-background/95 backdrop-blur sticky top-0 z-50">
      <div className="container mx-auto h-full px-4">
        <div className="flex items-center justify-between h-full">
          <Link to="/" className="flex items-center"><h1 className="text-2xl font-bold text-primary">BarberBook</h1></Link>
          <nav className="hidden md:flex items-center gap-6">
            <Link to="/book" className="text-foreground hover:text-primary transition-colors font-medium">Randevu Al</Link>
            {user && profile?.role === 'customer' && <Link to="/my-appointments" className="text-foreground hover:text-primary transition-colors font-medium">Randevularım</Link>}
            {user && profile?.role === 'admin' && <Link to="/admin/earnings" className="text-foreground hover:text-primary transition-colors font-medium">Admin Panel</Link>}
          </nav>
            ...
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              ...
              <SheetContent side="right" className="w-[300px]">
                <div className="flex flex-col gap-6 mt-8">
                  ...
                  {user && profile?.role === 'customer' && <Link to="/my-appointments" className="text-lg font-medium" onClick={() => setIsOpen(false)}>Randevularım</Link>}
                  {user && profile?.role === 'admin' && <Link to="/admin/earnings" className="text-lg font-medium" onClick={() => setIsOpen(false)}>Admin Panel</Link>}
