...
export default function AdminEarnings() {
  ...
  return (
    <div className="space-y-6">
      <div><h2 className="text-2xl font-bold mb-2">Kazançlar</h2><p className="text-muted-foreground">Günlük, haftalık ve aylık kazançlarınızı takip edin</p></div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Bugün</CardTitle><DollarSign className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">₺{earningsData.today}</div></CardContent></Card>
        <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Bu Hafta</CardTitle><Calendar className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">₺{earningsData.thisWeek}</div></CardContent></Card>
        <Card><CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2"><CardTitle className="text-sm font-medium">Bu Ay</CardTitle><TrendingUp className="h-4 w-4 text-muted-foreground" /></CardHeader><CardContent><div className="text-2xl font-bold">₺{earningsData.thisMonth}</div></CardContent></Card>
      </div>
      <Card><CardHeader><CardTitle className="flex items-center gap-2"><BarChart3 className="h-5 w-5" />Kazanç Analizi</CardTitle></CardHeader><CardContent><div className="flex justify-center gap-1 p-6">{earningsData.daily.map((d, i) => (<div key={i} className="flex flex-col items-center"><div className="bg-primary rounded-t-md w-12" style={{ height: `${Math.max(4, (d.amount / Math.max(...earningsData.daily.map(x => x.amount), 1)) * 100)}px` }}></div><span className="text-xs mt-1">{d.date}</span><span className="text-xs font-medium">₺{d.amount}</span></div>))}</div></CardContent></Card>
    </div>
  );
}
